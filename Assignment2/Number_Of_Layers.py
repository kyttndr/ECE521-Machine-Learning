...
def neural_network_model(data):
    ...
    hidden_2_w = tf.get_variable("hidden_Weight_2", [hidden_units, hidden_units], initializer=tf.contrib.layers.xavier_initializer(uniform=False))
    hidden_2_b = tf.Variable(tf.zeros([hidden_units]))
    layer2 = tf.nn.relu(tf.matmul(layer1, hidden_2_w) + hidden_2_b)
    hidden_3_w = tf.get_variable("hidden_Weight_3", [hidden_units, 10], initializer=tf.contrib.layers.xavier_initializer(uniform=False))
    hidden_3_b = tf.Variable(tf.zeros([10]))
    output = tf.matmul(layer2, hidden_3_w) + hidden_3_b
    return output, hidden_1_w, hidden_2_w, hidden_3_w

def train_neural_network(output):

    prediction, w1, w2, w3 = neural_network_model(output)
    weight_decay_loss = 3.0 * tf.pow(10.0, -4.0) * (tf.reduce_sum(tf.square(w1)) + tf.reduce_sum(tf.square(w2)) + tf.reduce_sum(tf.square(w3))) / 3.0
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 10 == 9:
                ...
                print "\tTraining Set: Error: %5.2f%%" %((1 - accuracy_train) * 100)
                print "\tValid Set: Error: %5.2f%%" %((1 - accuracy_valid) * 100)
            
        print "Optimization Finished!"
        print "Final Valid Error: %5.2f%%" %((1 - accuracy_valid) * 100)
